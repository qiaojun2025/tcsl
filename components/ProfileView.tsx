...
const ProfileView: React.FC<ProfileViewProps> = ({ stats, onUpgrade }) => {
  return (
    <div className="flex flex-col h-full bg-[#0A0A0A] text-white overflow-y-auto pb-24">
      {/* Header */}
      <div className="h-14 flex items-center justify-center px-4 relative mt-2 shrink-0">
        ...
        <button className="absolute right-4 w-8 h-8 flex items-center justify-center bg-white/5 rounded-full active:scale-90 transition-transform">
           <svg className="w-4 h-4 text-white/70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" strokeWidth="2.5" strokeLinecap="round"/></svg>
        </button>
      </div>
      ...
      <div className="px-4 space-y-8 mt-4">
        {/* Account Group */}
        <section>
          <h2 className="text-[11px] font-black text-white/30 uppercase ml-4 mb-2 tracking-[0.1em]">账户</h2>
          <div className="bg-[#161618] rounded-[22px] overflow-hidden divide-y divide-white/[0.03] border border-white/5 shadow-xl">
            ...
            <MenuItem 
              ...
              label="订阅" 
              onClick={onUpgrade}
              badge={stats.plan === 'Free' ? "Upgrade" : stats.plan}
            />
        ...
        {/* App Group */}
        <section>
          <h2 className="text-[11px] font-black text-white/30 uppercase ml-4 mb-2 tracking-[0.1em]">应用</h2>
          <div className="bg-[#161618] rounded-[22px] overflow-hidden divide-y divide-white/[0.03] border border-white/5 shadow-xl">
            <MenuItem icon={<AppearanceIcon />} label="外观" value="深色" showSelector />
        ...
        {/* About Group */}
        <section>
          <h2 className="text-[11px] font-black text-white/30 uppercase ml-4 mb-2 tracking-[0.1em]">关于</h2>
          <div className="bg-[#161618] rounded-[22px] overflow-hidden divide-y divide-white/[0.03] border border-white/5 shadow-xl">
        ...
        {/* Logout Button */}
        <button className="w-full bg-[#161618] text-[#FF1B6B] font-bold py-4 rounded-[22px] border border-white/5 shadow-xl active:scale-[0.98] transition-all flex items-center justify-center space-x-2">
...
const MenuItem = ({ icon, label, value, badge, showArrow, showSelector, onClick }: any) => (
  <div 
    onClick={onClick}
    className={`flex items-center px-4 py-4 space-x-3 active:bg-white/[0.03] transition-colors cursor-pointer ${onClick ? '' : 'pointer-events-none'}`}
  >
    <div className="w-6 h-6 flex items-center justify-center shrink-0 opacity-80">
      {icon}
    </div>
    <span className="flex-1 text-[15px] font-semibold text-white/90">{label}</span>
    {badge && (
    ...
    {value && <span className="text-[14px] text-white/30 font-medium">{value}</span>}
    {showArrow && <svg className="w-4 h-4 text-white/10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>}
    {showSelector && (
      <div className="flex flex-col items-center justify-center -space-y-1 text-white/10">
    ...
    {onClick && !showArrow && !badge && !showSelector && !value && <svg className="w-4 h-4 text-white/10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>}
  </div>
);
